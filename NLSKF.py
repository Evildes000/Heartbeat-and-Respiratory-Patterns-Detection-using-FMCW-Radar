...
config = FmcwSimpleSequenceConfig(
    ...
    frame_repetition_time_s=3,  # Frame repetition time
    chirp_repetition_time_s=0.001,  # Chirp repetition time

    num_chirps=2048,  # chirps per frame
    ...
    chirp=FmcwSequenceChirp(
        ...
        num_samples=512,  # samples per chirp
...
def record_frames(mat:np.ndarray, i_ant: int, i_frame: int):
    # record frame of antennas
    path_to_record = "masterarbeit-radar/antennas/antenna" + str(i_ant) + "/" + str(i_frame) + ".npy"
    np.save(path_to_record, mat) 


def load_frames(i_ant: int, i_frame: int):
    # load the recorded frame
    path_to_record = "masterarbeit-radar/antennas/antenna" + str(i_ant) + "/" + str(i_frame) + ".npy"
    mat = np.load(path_to_record)
    return mat 


"""
def max_peak_detection(mat:np.ndarray, range_bin_length:float):
    # mat: doppler_range_map
    # return indices of max peak
    # print(f"shape of mat : {np.shape(mat)}")
    skip = round(0.2/range_bin_length)

    max_value = np.max(np.abs(mat[:,skip:]))
    indices = np.argwhere(np.abs(mat[:,skip:]) == max_value)
    indices[0,1] = indices[0,1] + skip
    # print(indices)
    # (indices[0])[0] += skip 
    return indices[0,1]
"""
...
def mean_phase_elimination(mat:np.ndarray):
    """
    Calculate mean value of each column and substrate them from the mat
    so the static interference can be removed
    ...
    Return
    ------
    buffer : fast FFT matrices without static interference
    """
    [r,c] = np.shape(mat)
...
def max_peak_detection(mat:np.ndarray, range_freq:np.ndarray):
    ...
    # how many bins need to be skipped
    ...
    """
    plt.plot(range_freq[skip:], np.abs(eliminated_mat[1][skip:]))
    max_indix = np.argmax(np.abs(mat[1][skip:]))
    plt.plot(range_freq[skip:][max_indix], np.abs(eliminated_mat[1][skip:][max_indix]), "*")
    # plt.title("range fft")
    plt.xlabel("range/m", fontsize=12)
    plt.ylabel("amp", fontsize=12)
    plt.grid()
    ...
    max_coords = np.unravel_index(max_index, np.shape(eliminated_mat[:, skip:]))
    """
...
def coherent_averaging(mat:np.ndarray):
    ...
    None
    """
    # mat: range_fft of a frame
    # co_averaged: a 1*num_samples vector
    # calculate coherent_average of each frames
...
def phase_correlation(mat:np.ndarray, index:int, factor: float):
    ...
    """
    # to agrregate energy adjacent to the target and returns the recorvered displacement
    # this function also extract displacement of the target from phase changes
    # mat: range_fft
...
def phase_difference(disp:np.ndarray):
    buffer = np.zeros(len(disp))
    for i in np.arange(1, len(disp)):
        buffer[i] = disp[i] - disp[i-1]
    buffer[0] = disp[0]
...
def remove_baseline(disp: np.ndarray):
    ...
    # plt.title("baseline drift", fontsize=12)
    plt.xlabel("t/s", fontsize=12)
    plt.ylabel("Ampltude/mm", fontsize=12)
...
def map_real_to_complex(disp:np.ndarray):
    """
...
def seperate_analysis(mat:np.ndarray, case:str): 
    ...
    # used to transfer frequency to range after the fast time FFT
    disp_factor = lamda / (4*np.pi)
    # number of active Rx antennas
    num_antennas = 3
    # fft_freq = np.zeros(config.chirp.num_samples)

    # number of frames have been received of each active Rx antenna 
    frames = np.shape(mat)[0]
    print(f"number of frames is: {frames}")


    # buffers are for storage of fast FFT of frames of each antenna
    ...
    # create an instance to compute fast FFT
    ...
    for i_Ant in np.arange(num_antennas):
        for i_frame in np.arange(frames): 
            ...
            # range between two adjacent range bins
            range_freq = range_fft_result[0]

            # compute coherent averaging of fast FFT of a frame
    ...
    # used to store results that estimated from each antenna
    ...
    # boolean variables to control which funtions of NLS/KF should be called
    NLS_flag_1 = False
    NLS_flag_2 = False
    NLS_flag_3 = False
    ...
    KM_flag_2 = False
    KM_flag_3 = False
    # print(f"length of filtered_bufefr is: {len(filtered_buffer)}")
    ...
    while window_end <= frames:
        ...
        # combine energy in adjacent bins to the target bin together  
        ...
        # remove the baseline drift of displacement signals
        ...
        # compute differential signal for heart rate detection 
        ...
        #plot_all.plot_displacement(disp_1=diff_disp_1, disp_2=disp_2, disp_3=diff_disp_3)
        ...
        # filter displacement and differential displacement signals by band-pass filters
        ...
        # according to the paper ANLS&KF the real signals should be mapped into complex singals
        # but it turns out that there is no differnece without mapping      
        """
        complex_breath_wave_1 = map_real_to_complex(breath_wave_1)
        complex_heart_wave_1 = map_real_to_complex(heart_wave_1)

        complex_breath_wave_2 = map_real_to_complex(breath_wave_2)
        complex_heart_wave_2 = map_real_to_complex(heart_wave_2)

        complex_breath_wave_3 = map_real_to_complex(breath_wave_3)
        complex_heart_wave_3 = map_real_to_complex(heart_wave_3)
        """
        
        if NLS_flag_1 == False and NLS_flag_2 == False and NLS_flag_3 == False:
            fund_breath_1 = nls_funcs.NLS_breath_first(breath_wave_1)
            fund_breath_2 = nls_funcs.NLS_breath_first(breath_wave_2)
            fund_breath_3 = nls_funcs.NLS_breath_first(breath_wave_3)
            NLS_flag_1 = True
            NLS_flag_2 = True
            NLS_flag_3 = True
        else:
            # fund_breath = NLS_breath(breath_wave, fund_breath) 
            fund_breath_1 = nls_funcs.NLS_breath(breath_wave_1, fund_breath_1)
            fund_breath_2 = nls_funcs.NLS_breath(breath_wave_2, fund_breath_2)
            fund_breath_3 = nls_funcs.NLS_breath(breath_wave_3, fund_breath_3)
        

        # print(f"breath rate of antenna 1 is: {fund_breath_1}")
        # print(f"breath rate of antenna 2 is: {fund_breath_2}")
        # print(f"breath rate of antenna 3 is: {fund_breath_3}")

        estimated_heart_rates_1 = nls_funcs.NLS_heart(heart_wave_1)
        estimated_heart_rates_2 = nls_funcs.NLS_heart(heart_wave_2)
        estimated_heart_rates_3 = nls_funcs.NLS_heart(heart_wave_3)

        if KM_flag_1 == False and KM_flag_2 == False and KM_flag_3 == False:
            klf_1 = kf.kalman_filter(fund_breath_1)
            klf_2 = kf.kalman_filter(fund_breath_2)
            klf_3 = kf.kalman_filter(fund_breath_3)
            
            first_update_results_1 = klf_1.first_update(estimated_heartrates=estimated_heart_rates_1)
            first_update_results_2 = klf_2.first_update(estimated_heartrates=estimated_heart_rates_2)
            first_update_results_3 = klf_3.first_update(estimated_heartrates=estimated_heart_rates_3)

            updated_state_1 = first_update_results_1[0]
            updated_covariance_1 = first_update_results_1[1]
            heart_rates_1 = np.append(heart_rates_1, first_update_results_1[2])
            # buffer_hr_1 = buffer_hr_1 + first_update_results_1[2]    

            updated_state_2 = first_update_results_2[0]
            updated_covariance_2 = first_update_results_2[1]
            heart_rates_2 = np.append(heart_rates_2, first_update_results_2[2])
            # buffer_hr_2 = buffer_hr_2 + first_update_results_2[2]

            updated_state_3 = first_update_results_3[0]
            updated_covariance_3 = first_update_results_3[1]
            heart_rates_3 = np.append(heart_rates_3, first_update_results_3[2])
            # buffer_hr_3 = buffer_hr_3 + first_update_results_3[2]
            ...
            following_updated_results_1 = klf_1.following_updates(estimated_heartrates=estimated_heart_rates_1, current_state=updated_state_1, current_covariance=updated_covariance_1)
            following_updated_results_2 = klf_2.following_updates(estimated_heartrates=estimated_heart_rates_2, current_state=updated_state_2, current_covariance=updated_covariance_2)
            following_updated_results_3 = klf_3.following_updates(estimated_heartrates=estimated_heart_rates_3, current_state=updated_state_3, current_covariance=updated_covariance_3)

            updated_state_1 = following_updated_results_1[0]
            updated_covariance_1 = following_updated_results_1[1]
            # buffer_hr_1 = buffer_hr_1 + following_updated_results_1[2]
            heart_rates_1 = np.append(heart_rates_1, following_updated_results_1[2])

            updated_state_2 = following_updated_results_2[0]
            updated_covariance_2 = following_updated_results_2[1]
            # buffer_hr_2 = buffer_hr_2 + following_updated_results_2[2]
            heart_rates_2 = np.append(heart_rates_2, following_updated_results_2[2])

            updated_state_3 = following_updated_results_3[0]
            updated_covariance_3 = following_updated_results_3[1]
            # buffer_hr_3 = buffer_hr_3 + following_updated_results_3[2]
            heart_rates_3 = np.append(heart_rates_3, following_updated_results_3[2])

        # add estimated breathing and heart rates into the corresponding arrays
        breath_rates_1 = np.append(breath_rates_1, fund_breath_1)
        breath_rates_2 = np.append(breath_rates_2, fund_breath_2)
        breath_rates_3 = np.append(breath_rates_3, fund_breath_3)
    ...
    np.save("masterarbeit-radar/candidates_hrs_brs_combine_nls/"+case+"_breath_rates", breath_rates)
    np.save("masterarbeit-radar/candidates_hrs_brs_combine_nls/"+case+"_heart_rates", heart_rates)
